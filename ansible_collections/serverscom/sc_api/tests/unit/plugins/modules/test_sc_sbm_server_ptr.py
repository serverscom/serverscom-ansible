...
def test_find_ptr_match_by_ip_or_domain():
    # If either ip OR domain matches, the record is returned
    ptr = create_ptr_instance()
    # Match by IP only (domain doesn't match)
    result = list(ptr.find_ptr(PTR_RECORDS, "wrong.domain.com", "203.0.113.1"))
    assert PTR_RECORD_1 in result
