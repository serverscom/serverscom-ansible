    ...
    def run(self):
        ...
        if self.wait:
            ...
            while not ScSbmServerInfo._is_server_ready(server):
                ...
                server = self.api.get_sbm_servers(server["id"])
        server["changed"] = True
        return server
...
class ScSbmServerDelete:
    ...
    def wait_for_disappearance(self):
        ...
        while True:
            try:
                self.api.get_sbm_servers(self.server_id)
            except APIError404:
                return
            elapsed = time.time() - start_time
            if elapsed > self.wait:
    ...
    def create(self):
        ...
        if self.wait:
            ...
            while not self._is_network_active(network):
                ...
                network = self.api.get_sbm_server_network(self.server_id, network["id"])
        ...
        while True:
            try:
                network = self.api.get_sbm_server_network(
                    self.server_id, self.network_id
                )
            except APIError404:
                return
