...
6. **Parameters**: All modules require `token` and accept `endpoint`. Both support env var fallback via `env_fallback`: token falls back to `SERVERSCOM_API_TOKEN` then `SC_TOKEN`; endpoint falls back to `SERVERSCOM_API_URL`. Action modules often support `wait`/`update_interval`
...
8. **Auth env fallback**: All modules import `env_fallback` from `ansible.module_utils.basic` and use `"fallback": (env_fallback, ["SERVERSCOM_API_TOKEN", "SC_TOKEN"])` for token and `"fallback": (env_fallback, ["SERVERSCOM_API_URL"])` for endpoint. New modules must follow this pattern.

## Testing
- **Sanity/Unit**: `ansible-test sanity/units --requirements --python 3.13` (from `ansible_collections/serverscom/sc_api/`)
- **Integration**: Needs `tests/integration/integration_config.yml` with `sc_token` and `sc_endpoint` variables (see template). Tests pass these via `SERVERSCOM_API_TOKEN` and `SERVERSCOM_API_URL` environment variables per task. You should not run them.
